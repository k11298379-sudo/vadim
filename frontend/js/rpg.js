...
// ============================================================
// 11_admin_panel.js — Полная Админ-Панель управления RPG
// Управление золотом, уровнем, предметами и сбросом игроков
// ============================================================
  ...
  return `
    <div id="rpg-admin-floating-btn" class="fixed bottom-20 left-2.5 z-40">
      <button onclick="window.RPG.toggleAdminModal(true)"
        title="Панель администратора RPG"
    ...
    <div class="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3">
      <div class="w-full max-w-md max-h-[92vh] flex flex-col rounded-3xl bg-slate-900 border-2 border-amber-400/90 shadow-2xl text-white animate-scale-up overflow-hidden">
        ...
        <div class="p-3.5 border-b border-slate-700/80 flex items-center justify-between bg-slate-800/60 shrink-0">
          ...
          <button onclick="window.RPG.toggleAdminModal(false)"
            class="w-7 h-7 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 flex items-center justify-center font-bold text-xs">
            ✕
          </button>
        </div>
        ...
        <div class="flex border-b border-slate-700/80 bg-slate-950/40 p-1 gap-1 shrink-0">
          <button onclick="window.RPG.setAdminSubTab('actions')"
            class="flex-1 py-1.5 text-center text-xs font-black rounded-xl transition-all ${
              window._adminSubTab === 'actions'
        ...
        <div class="p-3.5 overflow-y-auto space-y-3 flex-1 text-xs">
          ${window._adminSubTab === "actions"
            ? renderAdminActionsTabHTML(players)
            : renderAdminSlotsTabHTML(activeTestSlot)
          }
        </div>
      </div>
    </div>
...
function renderAdminActionsTabHTML(players) {
  return `
    <!-- 1. Player Selector -->
    <div class="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 space-y-2">
      <div class="flex items-center justify-between">
        <span class="text-[10px] font-black uppercase text-amber-400 tracking-wider">🎯 Целевой Игрок:</span>
        <button onclick="window.RPG.refreshAdminPlayers()" class="text-[10px] text-sky-400 hover:underline flex items-center gap-1 font-bold">
          <span>🔄</span> Обновить список
        </button>
      </div>

      <select id="admin-target-select" onchange="window._adminSelectedTarget = this.value"
        class="w-full bg-slate-950 border border-slate-600 rounded-xl px-2.5 py-1.5 text-xs text-amber-200 font-bold focus:outline-none focus:border-amber-400">
        <option value="" ${!window._adminSelectedTarget ? 'selected' : ''}>👤 Текущий аккаунт (Я)</option>
        ${players.map(p => `
          <option value="${p.tg_id || p.user_id}" ${window._adminSelectedTarget == (p.tg_id || p.user_id) ? 'selected' : ''}>
            ${p.hero_icon || '⚔️'} ${p.name} [Ур. ${p.level} | 🪙 ${p.gold >= 1000 ? Math.round(p.gold/1000)+'k' : p.gold} | ID: ${p.tg_id || p.user_id}]
          </option>
        `).join('')}
      </select>

      <div class="flex items-center gap-2">
        <input type="text" id="admin-custom-target-input" placeholder="Или введите TG ID / User ID / @username"
          value="${window._adminSelectedTarget || ''}"
          onchange="window._adminSelectedTarget = this.value.trim()"
          class="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-2.5 py-1 text-[11px] text-slate-300 placeholder-slate-500">
        <button onclick="window.RPG.applyAdminCustomTarget()"
          class="px-2.5 py-1 bg-slate-700 hover:bg-slate-600 text-white rounded-xl text-[10px] font-bold">
          Выбрать
        </button>
      </div>
    </div>
    ...
    <div class="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 space-y-2">
      ...
      <div class="flex gap-2">
        <input type="number" id="admin-gold-custom-input" placeholder="Своя сумма золота"
          class="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-2.5 py-1 text-xs text-white">
        <button onclick="window.RPG.adminGiveGoldCustom()"
          class="px-3 py-1 bg-gradient-to-r from-amber-500 to-yellow-500 text-slate-950 rounded-xl font-black text-xs active:scale-95">
          Выдать
        </button>
      </div>
    </div>

    <!-- 3. Change Level (Уровень) -->
    <div class="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 space-y-2">
      <span class="text-[10px] font-black uppercase text-sky-400 tracking-wider flex items-center gap-1">
        <span>🆙</span> Изменить Уровень Героя (1..50)
      </span>
      <div class="grid grid-cols-4 gap-1.5">
      ...
      <div class="flex gap-2">
        <input type="number" id="admin-level-custom-input" min="1" max="50" placeholder="Уровень от 1 до 50"
          class="flex-1 bg-slate-950 border border-slate-700 rounded-xl px-2.5 py-1 text-xs text-white">
        <button onclick="window.RPG.adminSetLevelCustom()"
          class="px-3 py-1 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl font-black text-xs active:scale-95">
          Установить
        </button>
      </div>
    </div>

    <!-- 4. Give Custom Item (Выдать предмет) -->
    <div class="p-2.5 rounded-2xl bg-slate-800/80 border border-slate-700 space-y-2">
      <span class="text-[10px] font-black uppercase text-purple-400 tracking-wider flex items-center gap-1">
        <span>🎁</span> Выдать Предмет
      </span>
      <div class="grid grid-cols-2 gap-2">
        <div>
          <label class="text-[9px] text-slate-400 block mb-0.5">Редкость:</label>
          <select id="admin-item-rarity-select"
            class="w-full bg-slate-950 border border-slate-700 rounded-xl px-2 py-1 text-xs text-purple-300 font-bold focus:outline-none">
            <option value="common">⚪ Обычный</option>
            <option value="uncommon">🟢 Необычный</option>
            <option value="rare">🔵 Редкий</option>
            <option value="epic">🟣 Эпический</option>
            <option value="legendary" selected>🟠 Легендарный</option>
            <option value="mythic">🔴 Мифический</option>
            <option value="immortal">💖 Бессмертный</option>
          </select>
        </div>
        <div>
          <label class="text-[9px] text-slate-400 block mb-0.5">Уровень предмета (1..50):</label>
          <input type="number" id="admin-item-level-input" value="10" min="1" max="50"
            class="w-full bg-slate-950 border border-slate-700 rounded-xl px-2 py-1 text-xs text-white">
        </div>
      </div>
      <input type="text" id="admin-item-name-input" placeholder="Название предмета (оставьте пустым для случайного)"
        class="w-full bg-slate-950 border border-slate-700 rounded-xl px-2.5 py-1 text-[11px] text-white placeholder-slate-500">
      <button onclick="window.RPG.adminGiveItemSubmit()"
        ...
        ✨ Сгенерировать и Выдать в Инвентарь
      </button>
    </div>

    <!-- 5. Reset Progress (Обнулить прогресс) -->
    <div class="p-2.5 rounded-2xl bg-rose-950/40 border border-rose-600/50 space-y-1.5">
      <div class="flex items-center justify-between">
        <span class="text-[10px] font-black uppercase text-rose-400 tracking-wider flex items-center gap-1">
          <span>⚠️</span> Опасная Зона: Сброс
        </span>
      </div>
      <p class="text-[10px] text-rose-300/80 leading-tight">
        Полностью обнуляет выбранного игрока до 1 уровня, очищает экипировку и инвентарь до стартовых.
      </p>
      <button onclick="window.RPG.adminResetPlayerSubmit()"
        ...
        <span>🔄</span>
        <span>Обнулить Прогресс Выбранного Игрока</span>
      </button>
    </div>
  `;
}
...
window.RPG.setAdminSubTab = function(tab) {
  ...
  if (tab === "actions" && !window._adminPlayersList) {
...
window.RPG.adminGiveItemSubmit = async function() {
  try {
    ...
    const itemName = document.getElementById("admin-item-name-input")?.value?.trim() || null;
