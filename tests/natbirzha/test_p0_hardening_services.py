...
async def run() -> None:
    ...
    # NPC scaling is actually population-dependent: 1 -> 5 -> 20 -> 21+.
    async with async_session_factory() as session:
        q1 = await NPCReserveService.get_daily_quota(session)
        assert q1["scaling_factor"] == 1.0
        for uid in range(2002, 2006):
            await company(session, uid)
        q5 = await NPCReserveService.get_daily_quota(session)
        assert q5["scaling_factor"] == 0.8
        for uid in range(2006, 2021):
            await company(session, uid)
        q20 = await NPCReserveService.get_daily_quota(session)
        assert q20["scaling_factor"] == 0.3
        await company(session, 2021)
        q21 = await NPCReserveService.get_daily_quota(session)
        assert q21["scaling_factor"] == 0.1

    # The scaled NPC quota is enforced, not just reported.
    async with async_session_factory() as session:
        comp = await session.get(NatCompany, company_id)
        trade = await NPCReserveService.execute_npc_trade(
            session, comp, "grain", "BUY", q21["daily_quota_per_item"]
        )
        assert trade["success"] and trade["remaining_npc_quota"] == 0.0
        await session.commit()
    async with async_session_factory() as session:
        comp = await session.get(NatCompany, company_id)
        blocked = await NPCReserveService.execute_npc_trade(session, comp, "grain", "BUY", 1.0)
        assert blocked["success"] is False and blocked["reason"] == "npc_volume_limit"
        await session.rollback()
