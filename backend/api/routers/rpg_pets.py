...
async def hatch_pet(
    ...
    # Sort rates by highest probability to lowest or vice versa
    # Let's just do an explicit bucket
    # Actually, rates are out of 100.
    rates = [
...
@rpg_pets_router.post("/upgrade")
async def upgrade_pet(
    payload: Dict[str, Any] = Body(...),
    user: Optional[User] = Depends(get_optional_webapp_user),
    session: AsyncSession = Depends(get_db_session)
):
    """Merges 3 identical pets of the same stars to create a higher star pet."""
    user_id = user.id if user else 1
    char = await get_or_create_rpg_character(session, user_id=user_id)
    pet_uid = payload.get("pet_uid")
    ...
    uids_to_consume = [uid for uid in matching_uids if uid != pet_uid][:2] # Take 2 other pets
    ...
    # Upgrade costs 10 gems as a fee
    if getattr(char, "gems", 0) < 10:
        raise HTTPException(status_code=400, detail="Нужно 10 💎 для ритуала слияния!")
