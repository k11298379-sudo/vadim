...
async def get_npc_rates(session: AsyncSession = Depends(get_db_session)):
    ...
    for item_id in CANONICAL_ITEMS:
        item_quota = await NPCReserveService.get_daily_quota(session, item_id, "BUY")
        rates.append({**NPCReserveService.get_npc_quote(item_id), **item_quota})
...
async def trade_with_npc(
    ...
    if not result.get("success"):
        ...
        code = 409 if reason in {"npc_volume_limit", "market_restricted", "inventory_overflow"} else 400
