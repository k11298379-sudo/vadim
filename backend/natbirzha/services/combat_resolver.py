...
def _loss_fraction(winner: bool, margin: float) -> float:
    if winner:
        return round(min(0.12, max(0.03, 0.12 - 0.09 * margin)), 6)
    return round(min(0.22, max(0.08, 0.08 + 0.14 * margin)), 6)
...
def resolve_battle(
    ...
    *,
    ...
    maximum = max(attacker_score, defender_score, 1.0)
    margin = min(1.0, abs(attacker_score - defender_score) / maximum)
    attacker_loss_fraction = _loss_fraction(winner == "attacker", margin)
    defender_loss_fraction = _loss_fraction(winner == "defender", margin)
