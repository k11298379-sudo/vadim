...
from backend.natbirzha.services.pve_catalog import PVE_CATALOG_VERSION, PVE_CORPORATIONS
...
class PveService:
            ...
            result.append(
                {
                    ...
                    "approximate_strength": cls._snapshot_strength(target.unit_snapshot),
                    "strength_range": {
                        "min": round(cls._snapshot_strength(target.unit_snapshot) * 0.75),
                        "max": round(cls._snapshot_strength(target.unit_snapshot) * 1.25),
                    },
                    "conquered": conquered,
    ...
    async def scout_target(
        ...
        strength = cls._snapshot_strength(target.unit_snapshot)
        if drones >= 20:
        ...
        defender = ArmySnapshot(units=target.unit_snapshot)
    ...
    async def _check_access(
        session: AsyncSession, company: NatCompany, target: NatPveCorporation
    ) -> None:
        ...
        if existing is not None:
            raise PveWarError("already_conquered", "PvE corporation was already conquered")
    ...
    async def attack_target(
        ...
        *,
        ...
        if target is None:
            raise PveWarError("target_not_found", "PvE corporation not found")
        await cls._check_access(session, locked_company, target)
        ...
        territory_awarded = 0
        rewards: dict[str, Any] = {"cash": 0.0, "xp": 0, "resources": {}}
        if won and result.attacker_can_occupy:
            territory_awarded = target.territory_reward
            ...
            locked_company.cash = round(locked_company.cash + target.cash_reward, 2)
            locked_company.xp += target.xp_reward
            await cls._apply_resource_rewards(
                session, company.id, target.resource_rewards
            )
            victory = NatPveVictory(
                company_id=company.id,
                pve_corporation_id=target.id,
                battle_id=battle.id,
                reward_claimed=True,
