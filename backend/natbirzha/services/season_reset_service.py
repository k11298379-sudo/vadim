...
class SeasonResetService:
    ...
    async def execute(
        ...
        *,
        ...
        blueprints = [
            (company.user_id, company.name, company.specialization, is_creator_user(user))
            for company, user in rows
        ]
        for user_id, _name, _specialization, _is_creator in blueprints:
        ...
        for user_id, name, specialization, is_creator in blueprints:
            session.add(NatCompany(
                user_id=user_id,
                name=name,
                specialization=specialization,
                cash=200_000 if is_creator else nat_settings.STARTING_CASH,
            ))
