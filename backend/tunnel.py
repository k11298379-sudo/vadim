...
async def start_tunnel(port: int, timeout: float = 20.0) -> Optional[str]:
    """
    Launches a quick Cloudflare Tunnel pointing to localhost:{port}.
    Captures and returns the public https://xxxx.trycloudflare.com URL.
    """
    global _tunnel_proc, _tunnel_url, _stream_tasks

    binary = find_cloudflared()
    if not binary:
        logger.info("cloudflared executable not found. Running without public HTTPS tunnel.")
        return None
